...
class VPhoneState(Enum):
    ...
    QEMU = "qemu"                 # Running via QEMU (AMD64 bypass)
...
class VPhoneConfig:
    ...
    boot_mode: str = "tart"  # 'tart' (Apple Silicon) or 'qemu' (AMD64/Linux)
...
class VPhoneManager:
    ...
    def status(self) -> 'VPhoneStatus':
        ...
        try:
            ...
            if "qemu" in result.stdout.lower():
                return VPhoneStatus(state=VPhoneState.QEMU, vm_exists=True, tart_pid=tart_pid)
        except Exception:
            pass
    ...
    def _check_prerequisites(self) -> Dict[str, bool]:
        ...
        try:
            ...
            checks["apple_silicon"] = (arch == "arm64")
            checks["amd64_bypass_supported"] = (arch in ["x86_64", "amd64"])
        except Exception:
            checks["apple_silicon"] = False
            checks["amd64_bypass_supported"] = False
    ...
    def start(self, vnc: bool = True, wait_ssh: bool = True) -> bool:
        ...
        if not self.config.tart_binary and self.config.boot_mode == "tart":
            logger.error("❌ super-tart binary not found")
            return False

        if self.config.boot_mode == "qemu":
            return self._start_qemu(vnc, wait_ssh)
    ...
    def _start_qemu(self, vnc: bool = True, wait_ssh: bool = True) -> bool:
        """Start the vPhone using QEMU (AMD64/Linux bypass)"""
        qemu_script = Path(__file__).parent.parent.parent.parent.parent.parent / "scripts" / "vphone_qemu.sh"
        if not qemu_script.exists():
            logger.error(f"❌ QEMU boot script not found: {qemu_script}")
            return False

        logger.info(f"🚀 Starting VPhone via QEMU: {qemu_script}")
        try:
            self._tart_process = subprocess.Popen(
                ["bash", str(qemu_script)],
                stdout=subprocess.PIPE,
                stderr=subprocess.PIPE,
                preexec_fn=os.setsid,
            )
            logger.info(f"📱 VPhone QEMU process started (PID: {self._tart_process.pid})")
            if wait_ssh:
                return self.wait_for_ssh()
            return True
        except Exception as e:
            logger.error(f"❌ Failed to start VPhone QEMU: {e}")
            return False
