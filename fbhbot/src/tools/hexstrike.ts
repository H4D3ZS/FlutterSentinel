...
export async function hexstrikeAnalyze(target: string) {
    log.info(`FBH Tool: Delegating analysis to HexStrike for ${target}`);
    const analysis = await hexstrike.analyzeTarget(target);
    if (!analysis) return { status: "error", message: "HexStrike analysis failed." };
...
export async function hexstrikeAttack(target: string, tool: string, command: string) {
