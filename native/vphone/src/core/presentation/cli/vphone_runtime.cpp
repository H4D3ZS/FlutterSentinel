...
#include "vphone/HostProfile.hpp"
#include "vphone/Inventory.hpp"
#include "vphone/Runtime.hpp"
#include "vphone/vphone_core.h"
...
vphone_status_t vphone_runtime_init(const vphone_runtime_config_t *config,
  ...
  try {
    ...
    wrapper->request.dfu = config->dfu;

    // Detect host and load inventory
    vphone::HostProfile host = vphone::detectHostProfile();

    // Select bridge/emulator based on platform
    if (host.platform == "darwin" && host.architecture == "arm64") {
      wrapper->request.vzBridgeBinary =
          (repoRoot / "native/vphone/src/frontend/macos_vmapple/vzbridge" /
           (config->enable_native_ui ? "vphone" : "vphone-headless"))
              .string();
    } else {
      // Fallback to QEMU for everything else
#ifdef _WIN32
      wrapper->request.vzBridgeBinary = "qemu-system-aarch64.exe";
#else
      wrapper->request.vzBridgeBinary = "qemu-system-aarch64";
#endif
    }
