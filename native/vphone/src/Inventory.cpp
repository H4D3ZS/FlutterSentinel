...
const PlistValue *findDictValue(const PlistValue::DictStorage &dict,
                                const std::string &key) {
  const auto it = dict.find(key);
  return it == dict.end() ? nullptr : it->second.get();
}

std::string getString(const PlistValue::DictStorage &dict,
                      const std::string &key) {
...
const PlistValue::DictStorage &expectDict(const PlistValue *value,
...
const PlistValue::ArrayStorage &expectArray(const PlistValue *value,
                                            const std::string &context) {
...
SourceManifestSummary parseManifestSummary(const fs::path &manifestPath) {
  ...
  if (identitiesValue != nullptr &&
      identitiesValue->type() == PlistValue::Type::Array &&
      !identitiesValue->asArray().empty()) {
    const auto &identity =
        expectDict(identitiesValue->asArray().front().get(),
                   manifestPath.filename().string() + ".BuildIdentities[0]");
    const auto &info = expectDict(findDictValue(identity, "Info"),
                                  manifestPath.filename().string() + ".Info");
...
fs::path locateRestoreRoot(const fs::path &vmRoot,
                           const std::string &productTypeHint = "") {
  const fs::path expected = vmRoot / kExpectedRestoreDir;
  if (fs::exists(expected)) {
    return expected;
  }

  // Try to find a restore directory matching the hint
  std::string prefix = productTypeHint.empty() ? "iPhone" : productTypeHint;
  ...
  if (fs::exists(vmRoot)) {
    for (const auto &entry : fs::directory_iterator(vmRoot)) {
      ...
      if (name.find(prefix) == 0 &&
          name.find("_Restore") != std::string::npos) {
  ...
  // Fallback: If BuildManifest.plist exists directly in vmRoot, treat vmRoot as
  // the restore root
  if (fs::exists(vmRoot / "BuildManifest.plist")) {
    return vmRoot;
  }

  // Secondary fallback: Return vmRoot itself if it contains a Disk.img
  if (fs::exists(vmRoot / "Disk.img")) {
    return vmRoot;
  }

  throw std::runtime_error(
      "No restore directory or flattened firmware found under " +
      vmRoot.string());
}

FirmwareComponent makeComponent(const fs::path &restoreRoot,
                                const PlistValue::DictStorage &manifestDict,
                                const std::string &key) {
  const auto *componentValue = findDictValue(manifestDict, key);
  if (componentValue == nullptr ||
      componentValue->type() != PlistValue::Type::Dictionary) {
    return FirmwareComponent{key, {}, {}, false};
  }

  const auto &componentDict = componentValue->asDict();
  const auto &infoDict =
      expectDict(findDictValue(componentDict, "Info"), key + ".Info");
  const std::string relativePath = getString(infoDict, "Path");
  const fs::path absolutePath = relativePath.empty()
                                    ? fs::path{}
                                    : fs::absolute(restoreRoot / relativePath);
...
void appendJsonString(std::ostringstream &output, const std::string &key,
                      const std::string &value, bool trailingComma = true,
                      int indent = 2) {
  output << std::string(indent, ' ') << '"' << key << "\": \""
         << jsonEscape(value) << '"';
  if (trailingComma) {
    output << ',';
  }
...
ArtifactInventory loadInventory(const fs::path &repoRoot,
                                const fs::path &vmRootOverride) {
  ...
  const fs::path iphoneManifestPath =
      restoreRoot / "BuildManifest-iPhone.plist";
  const fs::path cloudManifestPath =
      vmRoot / "pcc-base" / "BuildManifest.plist";
  ...
  bool hasManifest = fs::exists(manifestPath);
  if (hasManifest) {
    const PlistValue manifest = parsePlistFile(manifestPath);
    const auto &root = expectDict(&manifest, "plist root");
    const auto &identities =
        expectArray(findDictValue(root, "BuildIdentities"), "BuildIdentities");
    if (!identities.empty()) {
      const auto &identity =
          expectDict(identities.front().get(), "BuildIdentity[0]");
      const auto &info =
          expectDict(findDictValue(identity, "Info"), "BuildIdentity[0].Info");
      const auto &manifestDict = expectDict(findDictValue(identity, "Manifest"),
                                            "BuildIdentity[0].Manifest");
      ...
      for (const std::string &key :
           {"KernelCache", "DeviceTree", "LLB", "iBSS", "iBEC",
            "RestoreRamDisk", "SEP", "StaticTrustCache"}) {
        summary.manifestComponents.emplace(
            key, makeComponent(restoreRoot, manifestDict, key));
      }
      inventory.buildIdentity = std::move(summary);
    }
  } else {
    inventory.buildIdentity.deviceClass = "iPhone13,2";
    inventory.buildIdentity.buildNumber = "26.3";
    inventory.notes.push_back(
        "Warning: BuildManifest.plist not found. Using discovery fallback.");
  }
...
RuntimeMetadata deriveRuntimeMetadata(const ArtifactInventory &inventory) {
  ...
  metadata.bootIdentityBuild =
      inventory.buildIdentity.productBuildVersion.empty()
  ...
  metadata.isHybrid =
      !metadata.bootIdentityVersion.empty() &&
      !metadata.userspaceVersion.empty() &&
      (metadata.bootIdentityVersion != metadata.userspaceVersion ||
       metadata.bootIdentityBuild != metadata.userspaceBuild);
  return metadata;
}
...
std::string renderInventoryHuman(const ArtifactInventory &inventory) {
  ...
  output << "Host: " << inventory.host.platform << '/'
         << inventory.host.architecture
         << " accel=" << inventory.host.accelerator << "\n";
  output << "Device class: " << inventory.buildIdentity.deviceClass << "\n";
  output << "Build identity: " << inventory.buildIdentity.buildNumber << "\n";
  output << "Manifest product version: "
         << inventory.buildIdentity.productVersion << "\n";
  output << "Manifest product build: "
         << inventory.buildIdentity.productBuildVersion << "\n\n";

  const RuntimeMetadata metadata = deriveRuntimeMetadata(inventory);
  output << "Runtime metadata:\n";
  output << "- Boot identity: " << metadata.bootIdentityVersion << " / "
         << metadata.bootIdentityBuild << " (" << metadata.bootIdentitySource
         << ")\n";
  output << "- Userspace: " << metadata.userspaceVersion << " / "
         << metadata.userspaceBuild << " (" << metadata.userspaceSource
         << ")\n";
  output << "- Hybrid: " << (metadata.isHybrid ? "yes" : "no") << "\n\n";

  output << "Source manifests:\n";
  output << "- iPhone source: " << inventory.iphoneSource.productVersion
         << " / " << inventory.iphoneSource.productBuildVersion;
  if (!inventory.iphoneSource.deviceClass.empty()) {
    output << " (" << inventory.iphoneSource.deviceClass << ", "
           << inventory.iphoneSource.buildNumber << ')';
  }
  output << "\n";
  output << "- PCC source: " << inventory.cloudSource.productVersion << " / "
         << inventory.cloudSource.productBuildVersion;
  if (!inventory.cloudSource.deviceClass.empty()) {
    output << " (" << inventory.cloudSource.deviceClass << ", "
           << inventory.cloudSource.buildNumber << ')';
  }
  output << "\n\n";

  output << "Core files:\n";
  output << "- Disk: " << (inventory.exists.at("disk_image") ? "yes" : "no")
         << " -> " << inventory.diskImage << "\n";
  output << "- AVPBooter: " << (inventory.exists.at("avpbooter") ? "yes" : "no")
         << " -> " << inventory.avpBooter << "\n";
  output << "- AVPSEPBooter: "
         << (inventory.exists.at("avpsepbooter") ? "yes" : "no") << " -> "
         << inventory.avpSepBooter << "\n";
  output << "- Ramdisk dir: "
         << (inventory.exists.at("ramdisk_root") ? "yes" : "no") << " -> "
         << inventory.ramdiskRoot << "\n\n";

  output << "Manifest components:\n";
  for (const auto &[key, component] :
       inventory.buildIdentity.manifestComponents) {
    output << "- " << key << ": " << (component.exists ? "yes" : "no") << " -> "
...
std::string renderInventoryJson(const ArtifactInventory &inventory) {
  ...
  appendJsonString(output, "architecture", inventory.host.architecture, true,
                   4);
  ...
  appendJsonString(output, "device_class", inventory.buildIdentity.deviceClass,
                   true, 4);
  appendJsonString(output, "build_number", inventory.buildIdentity.buildNumber,
                   true, 4);
  appendJsonString(output, "restore_behavior",
                   inventory.buildIdentity.restoreBehavior, true, 4);
  appendJsonString(output, "variant", inventory.buildIdentity.variant, true, 4);
  appendJsonString(output, "product_version",
                   inventory.buildIdentity.productVersion, true, 4);
  appendJsonString(output, "product_build_version",
                   inventory.buildIdentity.productBuildVersion, true, 4);
  appendJsonString(output, "semantic_role",
                   inventory.buildIdentity.semanticRole, false, 4);
  output << "  },\n";

  const RuntimeMetadata metadata = deriveRuntimeMetadata(inventory);
  output << "  \"runtime_metadata\": {\n";
  appendJsonString(output, "boot_identity_version",
                   metadata.bootIdentityVersion, true, 4);
  appendJsonString(output, "boot_identity_build", metadata.bootIdentityBuild,
                   true, 4);
  appendJsonString(output, "boot_identity_source", metadata.bootIdentitySource,
                   true, 4);
  appendJsonString(output, "userspace_version", metadata.userspaceVersion, true,
                   4);
  appendJsonString(output, "userspace_build", metadata.userspaceBuild, true, 4);
  appendJsonString(output, "userspace_source", metadata.userspaceSource, true,
                   4);
  ...
  appendJsonString(output, "product_version",
                   inventory.iphoneSource.productVersion, true, 6);
  appendJsonString(output, "product_build_version",
                   inventory.iphoneSource.productBuildVersion, true, 6);
  appendJsonString(output, "device_class", inventory.iphoneSource.deviceClass,
                   true, 6);
  appendJsonString(output, "build_number", inventory.iphoneSource.buildNumber,
                   false, 6);
  output << "    },\n";
  output << "    \"pcc\": {\n";
  appendJsonString(output, "path", inventory.cloudSource.path, true, 6);
  appendJsonString(output, "product_version",
                   inventory.cloudSource.productVersion, true, 6);
  appendJsonString(output, "product_build_version",
                   inventory.cloudSource.productBuildVersion, true, 6);
  appendJsonString(output, "device_class", inventory.cloudSource.deviceClass,
                   true, 6);
  appendJsonString(output, "build_number", inventory.cloudSource.buildNumber,
                   false, 6);
  output << "    }\n";
  output << "  },\n";
