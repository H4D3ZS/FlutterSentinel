...
export async function performStrategicAnalysis(memory: VectorMemoryManager, target: string) {
    ...
    for (const pattern of patterns) {
        const matchingSteps = pattern.steps.filter(step =>
            targetFindings.some(f => f.content.toUpperCase().includes(step) || f.title.toUpperCase().includes(step))
        );
