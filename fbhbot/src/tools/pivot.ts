 ...
 */
export async function establishPivot(memory: VectorMemoryManager, context: {
    target: string;
    credentials?: any;
 ...
 */
export async function tunnelCommand(memory: VectorMemoryManager, sessionId: string, command: string) {
    const pivots = await memory.getPivots();
    const session = pivots.find(p => p.id === sessionId);

    if (!session) throw new Error("Active pivot session not found in Global Brain.");
    if (session.status !== "active") throw new Error(`Pivot session ${sessionId} is ${session.status}.`);
