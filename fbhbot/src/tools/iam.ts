...
export async function iamAudit(context: {
    ...
    const matchAction = (policyAction: string, requiredAction: string): boolean => {
    ...
    for (const policy of context.discovered_policies) {
        ...
        for (const pattern of escalationPatterns) {
            const matches = pattern.actions.every(required =>
                allActions.some(allowed => matchAction(allowed, required))
            );
