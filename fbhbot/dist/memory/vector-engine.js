...
`);
    ...
    async updateSettings(userId, settings) {
        const stmt = this.db.prepare("INSERT OR REPLACE INTO settings (user_id, key_name, value) VALUES (?, ?, ?)");
    ...
    seedPlaybooks() {
        ...
        const insertStmt = this.db.prepare("INSERT INTO playbooks (id, user_id, name, description, instructions) VALUES (?, ?, ?, ?, ?)");
        for (const pb of defaultPlaybooks) {
            const result = checkStmt.get(pb.id);
            if (result.count === 0) {
    ...
    async createSchedule(schedule) {
        const stmt = this.db.prepare("INSERT INTO schedules (id, user_id, target, frequency, next_run) VALUES (?, ?, ?, ?, ?)");
    ...
    async updateScheduleNextRun(id, nextRun) {
        const stmt = this.db.prepare("UPDATE schedules SET next_run = ? WHERE id = ?");
        stmt.run(nextRun, id);
    }
    // ... [finding methods]
    async storeMission(mission) {
        const stmt = this.db.prepare("INSERT INTO missions (id, user_id, target, goal, status, state) VALUES (?, ?, ?, ?, ?, ?)");
    ...
    async storeFinding(finding, apiKey) {
        const metadataStr = JSON.stringify(finding.metadata || {});
        const insertStmt = this.db.prepare("INSERT INTO findings (title, content, severity, metadata) VALUES (?, ?, ?, ?)");
        ...
        if (apiKey) {
            try {
                ...
                if (embedding) {
                    const vecStmt = this.db.prepare("INSERT INTO findings_embeddings (finding_id, embedding) VALUES (?, ?)");
    ...
    async searchSemantic(query, limit = 5, apiKey) {
        if (!apiKey) {
...
LIMIT ?
  `);
    ...
    async searchRecent(query, limit = 5) {
    ...
    async getMostRecentMission(target) {
    ...
    async updatePivotStatus(id, status) {
