 ...
 */
export async function generateBountyReport(mission: Mission): Promise<{ report: string; filename: string }> {
    ...
    for (const stage of completedStages) {
        ...
        if (stage.result.findings && Array.isArray(stage.result.findings)) {
            stage.result.findings.forEach((f: any) => {
...
export async function submitBounty(mission: Mission, platform: string = "FBH_Submission_Bus") {
