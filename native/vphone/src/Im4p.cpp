...
bool hasPrefix(const std::vector<std::uint8_t>& input, const char* prefix) {
    ...
    return input.size() >= length && std::equal(prefix, prefix + length, input.begin());
}

std::size_t parseLength(const std::vector<std::uint8_t>& input, std::size_t& cursor) {
    ...
    if (count == 0 || count > sizeof(std::size_t) || cursor + count > input.size()) {
...
std::string readString(const std::vector<std::uint8_t>& input, const DerNode& node) {
    return std::string(
        reinterpret_cast<const char*>(input.data() + node.contentOffset),
        reinterpret_cast<const char*>(input.data() + node.contentOffset + node.contentLength)
    );
}

Im4pPayload parseIm4pSequence(const std::vector<std::uint8_t>& input, const DerNode& sequence) {
    ...
    if (fourccNode.tag != 0x16U || descriptionNode.tag != 0x16U || payloadNode.tag != 0x04U) {
    ...
    result.payload.assign(
        ...
        input.begin() + static_cast<std::ptrdiff_t>(payloadNode.contentOffset + payloadNode.contentLength)
    );
    ...
    if (type == "IMG4") {
        while (childCursor < root.contentOffset + root.contentLength) {
            ...
            try {
                ...
                childCursor = probe + (child.contentOffset + child.contentLength - child.headerOffset);
            }
        }
...
std::vector<std::uint8_t> maybeDecompressBvx2(const std::vector<std::uint8_t>& input, bool& decompressed, std::string& note) {
    ...
    for (int attempt = 0; attempt < 6; ++attempt) {
        ...
        const std::size_t written = compression_decode_buffer(
            output.data(),
            output.size(),
            input.data(),
            input.size(),
        ...
        if (written > 0) {
            ...
            decompressed = true;
            note = "Decompressed bvx2/LZFSE payload with Apple compression framework.";
            return output;
        }
    ...
    note = "Detected bvx2/LZFSE payload but native decompression failed on this host.";
    return input;
#else
    note = "Detected bvx2/LZFSE payload; native decompression is not implemented for this host yet.";
    return input;
#endif
}
