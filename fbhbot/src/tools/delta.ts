...
export async function performDeltaAnalysis(memory: VectorMemoryManager, target: string) {
    ...
    const deltas = [];
    if (latest.title !== prior.title) deltas.push("Change in vulnerability title/category.");
    if (latest.severity !== prior.severity) deltas.push(`Severity shift from ${prior.severity} to ${latest.severity}.`);

    // Simplified diff
    const contentChanged = latest.content.length !== prior.content.length;
    if (contentChanged) deltas.push("Significant change in investigation content/results.");
