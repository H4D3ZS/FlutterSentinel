...
export async function generateSpearphishingCampaign(context) {
    ...
    let selectedSubject = "Urgent: Action Required";
    let selectedBody = "Please see the attached document.";
    let selectedAttachment = "Document.pdf";
    if (apiKey) {
        try {
            ...
            log.error(`Generative AI lure synthesis failed: ${err}. Falling back to templates.`);
            // Fallback would go here, but for brevity we'll just use the defaults initialized above
        }
    }
