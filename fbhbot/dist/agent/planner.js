...
export class MissionPlanner {
    ...
    async createMission(target, goal) {
    ...
    async injectStage(missionId, afterStageId, stage) {
        ...
        const index = mission.state.stages.findIndex((s) => s.id === afterStageId);
        if (index !== -1) {
    ...
    async updateStage(missionId, stageId, status, result) {
        const mission = await this.memory.getMission(missionId);
        if (!mission)
            throw new Error("Mission not found");
        const stage = mission.state.stages.find((s) => s.id === stageId);
        if (stage) {
            stage.status = status;
            if (result)
                stage.result = result;
        ...
        const allDone = mission.state.stages.every((s) => s.status === 'completed' || s.status === 'failed');
