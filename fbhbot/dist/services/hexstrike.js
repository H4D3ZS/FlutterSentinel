...
export class HexStrikeService {
    ...
    async analyzeTarget(target) {
        ...
        try {
            ...
            if (!response.ok)
                throw new Error(`HexStrike API error: ${response.statusText}`);
    ...
    async selectTools(targetProfile) {
    ...
    async executeWithRecovery(toolName, command, parameters = {}) {
        ...
        try {
            ...
            if (!response.ok)
                throw new Error(`HexStrike execution failed: ${response.statusText}`);
            const data = await response.json();
            return data.result;
        }
        catch (error) {
